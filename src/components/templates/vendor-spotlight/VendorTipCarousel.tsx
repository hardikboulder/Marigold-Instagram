...
import type { VendorAccentColor } from "./VendorFeaturePost";
...
export interface VendorTipCarouselProps {
  vendorCategory: string;
  vendorName: string;
  vendorHandle: string;
  tips: VendorTip[];
  accentColor?: VendorAccentColor;
  /** 1 = cover, 2-5 = tips, 6 = close. Default 1. */
  slideIndex?: number;
}
...
export function VendorTipCarousel({
  vendorCategory,
  vendorName,
  vendorHandle,
  tips,
  ...
  if (slideIndex >= 6) {
    return <CloseSlide vendorHandle={vendorHandle} accent={accent} />;
  }
  ...
  return (
    <TipSlide tipNumber={tipNumber} tip={tip} accent={accent} />
  );
}
