/**
 * Content calendar CRUD — backed by localStorage via `local-store.ts`.
 *
 * Mirrors the `content_calendar` schema in SPEC §4.4, minus the Supabase /
 * PostgREST-specific bits. UUIDs are minted with `crypto.randomUUID()`.
 */
...
function readAll(): CalendarItem[] {
  ...
  // Backfill `pillar` on items written before the pillars migration so the
  // calendar still renders for users with pre-existing localStorage data.
  return raw.map((item) => {
...
function newId(): string {
  ...
  // Fallback for older runtimes — sufficient for local dev, not for prod.
